...
class client_thread(threading.Thread):
    ...
    def run(self):
        global timer
        self.start_timer = int(timeit.default_timer())
        self.recv_data()
        print ("the file name asked is:", self.file_name)
        '''print ("The client's port is:", self.conn)'''
        if self.file_name.startswith('/') and self.file_name.endswith('.ico'):
            self.send_quit_to_client()
        else:
            self.get_file_path()
        self.stop_timer = int(timeit.default_timer())
        timer = self.stop_timer - self.start_timer
        ...
        for sock in read_sock:
            if sock == s:
                ...
                if 'User-Agent:' in data:
                    b = 1
                    new_thread = http_one_zero(args = (sock,root_dir,file_name,method,http_version,b,))
                    new_thread.start()
                    ...
                    if http_version == 1.0:
                        ...
                        new_thread = client_thread(args = (conn,root_dir,file_name,method,http_version,))
                        new_thread.start()
                        print (timer)
